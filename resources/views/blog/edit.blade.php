...
<div class="mx-auto mt-8 w-10/12">
    ...
    <form action="{{ route('update', $myblog) }}" method="post" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
        @csrf
        @method('patch')
        <div class="mb-4">
        ...
        <div class="mb-6">
            ...
            <textarea
                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                id="content" name="content" rows="11" placeholder="コンテンツを入力してください">{{ old('content', $myblog->content) }}</textarea>
        </div>
        <div class="flex items-center justify-between">
            <a class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline"
                href="{{ route('show', $myblog) }}">キャンセル</a>
            <button
